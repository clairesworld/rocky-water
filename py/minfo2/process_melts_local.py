...
git pull
python3 py/minfo2/process_melts_local.py
"""
...
restart = input('Restart first loop at star (no spaces): ')
...
for ce in core_eff:
    for Xf in X_ferric:
        for T_of_interest in T_of_interests:
            ...
            # calculate mantle fo2 only, don't run melts
            mfug.fo2_from_local(output_parent_path, core_efficiency=ce, X_ferric=Xf, alphamelts_path=alphamelts_path,
                                ...
                                verbose=False,
                                restart = restart
                                # restart='2MASS19155319+4437283',  # no spaces in star
            ...
            # mfug.create_isothermal_csv(output_parent_path, T=T_of_interest, P=1, Xfer=Xf, coreeff=ce,
            #                            dropnan=True, exclude_silica=True, verbose=True)

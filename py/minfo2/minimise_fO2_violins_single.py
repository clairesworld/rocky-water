...
from py.useful_and_bespoke import colorize
...
for axis in [ax2]:
    ...
    if axis == ax2:
        chem_in = '/home/claire/Works/min-fo2/sean_fastchem/table_FMQ_Venus_atm_COSH_chem.dat'
        norm_in = '/home/claire/Works/min-fo2/sean_fastchem/table_FMQ_Venus_atm_COSH_norm.dat'
    # if axis == ax3:
    ...
    colours = ['tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:purple', 'tab:pink', 'tab:brown', 'tab:olive',
               'tab:cyan', 'tab:grey']
    colours = colorize(np.arange(len(species_list)), cmap='Greys', vmin=-3)[0]
...
ax2.tick_params(axis='y', which='both', pad=pad_size, labelsize=label_size, direction='in', top=True, right=True)

ax2.tick_params(axis='both', which='both', pad=pad_size, labelsize=label_size, direction='in', top=True, right=True)
...
ax2.set_ylabel('log($\dfrac{X_i}{X_{i, FMQ}}$)', fontsize=label_size)

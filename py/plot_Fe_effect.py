...
def plot_XFe_dependence(M_p=1, Tp=1600, x_Fe=None, sigma=1, labelsize=16, figsize=(4, 4),
                        legsize=10, ticksize=12, xpad=20, ypad=20, title='', earth=None,
                        cmap_phases='tab20', linec='xkcd:navy', linew=2, xlims=None,
                        save=True, fig=None, ax=None, **kwargs):
    """ make plot for several masses showing mantle structure and relative water in each zone"""

    if ax is None:
        fig, ax = plt.subplots(1, 1, figsize=figsize)
    if x_Fe is None:
        x_Fe = [0.5, 0.8, 0.9999999]
    ...
    for ii in range(len(y_names)):
        ...
        fig, ax = plotpx.compare_pop_fillbetween(dirs, 'core_eff*-1 + 1', y_names[ii], x_scale=1, y_scale=p.TO ** -1,
                                                 xlabel='Mantle Fe fraction', ylabel='Water capacity (OM)', title=title,
...
plot_XFe_dependence(M_p=1, Tp=1600, sigma=1, title=r'1 $M_\oplus$, 1600 K', save=True, earth=earth)
